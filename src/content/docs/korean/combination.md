...
**겹모음**(ㅘ, ㅙ, …), **겹받침**(ㄳ, ㄵ, …), **종성→다음 초성** 변환 규칙·테이블은 [한글 조합 규칙(겹모음·겹받침)](/docs/korean/combination-rules/)를 참고한다. 2벌식·3벌식 키 매핑·상태 전이 차이는 [2벌식 알고리즘](/docs/korean/2-set-algorithm/), [3벌식 알고리즘](/docs/korean/3-set-algorithm/)을 참고한다.
